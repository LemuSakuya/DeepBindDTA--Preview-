...
def test(model, dataloader):
    model.eval()
    preds = []
    # Disable tqdm to prevent buffer issues in GUI subprocess
...
predModel.load_state_dict(torch.load(model_fromTrain, map_location=torch.device('cpu')))
...
print(f'Predict over {save_path}.', flush=True)
